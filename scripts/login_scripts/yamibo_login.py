...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Log in to Yamibo and save session cookies")
    p.add_argument(
        "-u",
        "--user-name",
        type=str,
        required=True,
        help="Yamibo account user name",
    )
    p.add_argument(
        "-p",
        "--password",
...
def _url_changed(url: str, before: str) -> bool:
    return url != before
...
def login(
    ...
    for i in range(1, attempt + 1):
        ...
        page.goto(LOGIN_URL)
        page.fill("#loginform-username", username)
        page.fill("#loginform-password", password)
        before_url = page.url
        page.click('button[name="login-button"]')
        try:
            page.wait_for_url(
                lambda url, before_url=before_url: _url_changed(url, before_url),
                timeout=15000,
            )
        except Exception as e:
            print(f"[auth] No URL change after login attempt {i}: {e}")
...
def main() -> None:
    ...
    state_file = args.data_dir / "yamibo" / COOKIES_FILENAME
