...
class CiyuanjiExporter(BaseExporter):
    """
    ...
    def _build_epub_chapter(
        ...
        img_map = self._collect_img_map(extras)

        for i, raw in enumerate(paragraphs.splitlines(), start=1):
            parts.append(f"<p>{escape(raw.strip())}</p>")
            if not self._include_picture:
                continue
            for img_url in img_map.get(i, []):
                local_path = self._download_image(img_url, img_dir, on_exist="skip")
                if not local_path:
                    self.logger.debug("Image skipped or failed: %s", img_url)
                    continue
                filename = book.add_image(local_path)
                parts.append(
                    self._IMAGE_WRAPPER.format(
                        img=f'<img src="../Images/{filename}" />'
                    )
                )
    ...
    def _collect_img_map(extras: dict[str, Any]) -> dict[int, list[str]]:
        """
        Collect a mapping from paragraphIndex -> list of image URLs
        from the given `extras` dict.
        """
        ...
        for item in img_list:
            if not isinstance(item, dict):
                continue
            index = item.get("paragraphIndex")
            url = item.get("imgUrl")
            if isinstance(index, int) and isinstance(url, str):
                img_map.setdefault(index, []).append(url)
