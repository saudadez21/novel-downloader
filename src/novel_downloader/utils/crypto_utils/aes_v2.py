#!/usr/bin/env python3
"""
novel_downloader.utils.crypto_utils.aes_v2
------------------------------------------
...
def _rijndael_key_setup_enc(key: bytes) -> tuple[list[int], int]:
    """
    Return (rk, Nr) where rk is encryption key schedule of length 4*(Nr+1).
    """
    key_bits = len(key) * 8
...
def _rijndael_key_setup_dec(key: bytes) -> tuple[list[int], int]:
    """
    Return (drk, Nr) where drk is decryption key schedule of length 4*(Nr+1).
    """
...
class AES:
    ...
    def encrypt_block(self, plaintext: bytes) -> bytes:
        ...
        idx = 4
        # Nr-1 full rounds with MixColumns via Te0..Te3
        for _ in range(1, Nr):
            t0 = (
                Te0[(s0 >> 24) & 0xFF]
                ^ Te1[(s1 >> 16) & 0xFF]
        ...
        struct.pack_into(">IIII", self._out_buf, 0, c0, c1, c2, c3)
    ...
    def decrypt_block(self, ciphertext: bytes) -> bytes:
        ...
        idx = 4
        # Nr-1 full inverse rounds (Td tables)
        for _ in range(1, Nr):  # unrolled can speedup x1.1
            t0 = (
                Td0[(s0 >> 24) & 0xFF]
                ^ Td1[(s3 >> 16) & 0xFF]
                ^ Td2[(s2 >> 8) & 0xFF]
        ...
        # Final inverse round (no MixColumns)
        p0 = (
        ...
        struct.pack_into(">IIII", self._out_buf, 0, p0, p1, p2, p3)
        return bytes(self._out_buf)
...
class AES_CBC(AES):
    ...
    def reset(self, iv: bytes | None = None) -> None:
        """
        Reset the CBC state.
        """
        self.iv = iv or bytes(16)
    ...
    def decrypt(self, ciphertext: bytes) -> bytes:
        ...
        for i in range(0, len(ciphertext), self.block_size):
            ...
            pt = bytes(a ^ b for a, b in zip(dec, prev, strict=False))
            plaintext += pt
            prev = block
        self.iv = prev
        return plaintext
