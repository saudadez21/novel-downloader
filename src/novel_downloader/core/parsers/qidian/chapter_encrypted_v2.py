#!/usr/bin/env python3
"""
novel_downloader.core.parsers.qidian.chapter_encrypted_v2
---------------------------------------------------------
...
class Rule:
    ...
    transform_flip_x: bool = False
    append_start_char: str | None = None
    append_end_char: str | None = None
    append_start_attr: str | None = None
    append_end_attr: str | None = None
...
class Rules:
    ...
    # e.g., sy["sy-3"] -> Rule
    sy: dict[str, Rule] = field(default_factory=dict)
    # e.g., p_rules["p3"]["i"] -> Rule
    p_rules: dict[str, dict[str, Rule]] = field(default_factory=dict)
...
def parse_encrypted_chapter(
    ...
    try:
        ...
        rules = parse_css_rules(css_str)
        paragraphs_str, refl_list = render_visible_text(raw_html, rules)
        if parser.save_font_debug:
...
def _only_tag(selector: str) -> str | None:
    ...
    """
    sel = selector.strip()
    # If it has spaces, take the rightmost simple selector (after the last space)
    last = sel.split()[-1]
    # Drop ::pseudo
    last = last.split("::", 1)[0]
...
def _merge_rules(base: Rule, extra: Rule) -> Rule:
    # OR booleans; prefer 'extra' if it sets explicit char/attr fields
    return Rule(
        delete_all=base.delete_all or extra.delete_all,
        delete_first=base.delete_first or extra.delete_first,
        transform_flip_x=base.transform_flip_x or extra.transform_flip_x,
        append_start_char=extra.append_start_char
        if extra.append_start_char is not None
        else base.append_start_char,
        append_end_char=extra.append_end_char
        if extra.append_end_char is not None
        else base.append_end_char,
        append_start_attr=extra.append_start_attr
        if extra.append_start_attr is not None
        else base.append_start_attr,
        append_end_attr=extra.append_end_attr
        if extra.append_end_attr is not None
        else base.append_end_attr,
    )
...
def parse_css_rules(css_str: str) -> Rules:
    ...
    for rule in stylesheet:
        ...
        for decl in decls:
            ...
            if name == "font-size" and value == "0":
                if "::first-letter" in selector_raw:
                    tmp["delete_first"] = True
                else:
                    tmp["delete_all"] = True
            elif name == "transform" and value.lower().replace(" ", "") == "scalex(-1)":
                tmp["transform_flip_x"] = True
            elif name == "order":
                # keep numeric; non-numeric orders ignored
                ...
                if "::after" in selector_raw:
                    if "attr(" in value:
                        ...
                        tmp["append_end_attr"] = attr
                    else:
                        tmp["append_end_char"] = value.strip("\"'")
                elif "::before" in selector_raw:
                    if "attr(" in value:
                        attr = value.split("attr(", 1)[1].split(")", 1)[0]
                        tmp["append_start_attr"] = attr
                    else:
                        tmp["append_start_char"] = value.strip("\"'")

        new_rule = Rule(
            delete_all=tmp.get("delete_all", False),
            delete_first=tmp.get("delete_first", False),
            transform_flip_x=tmp.get("transform_flip_x", False),
            append_start_char=tmp.get("append_start_char"),
            append_end_char=tmp.get("append_end_char"),
            append_start_attr=tmp.get("append_start_attr"),
            append_end_attr=tmp.get("append_end_attr"),
        )
        ...
        if selector_raw.startswith(".sy-"):
            ...
            old = rules.sy.get(key, Rule())
            rules.sy[key] = _merge_rules(old, new_rule)
            ...
            tag = _only_tag(right)
            if tag:
                old = rules.p_rules.setdefault(p_cls, {}).get(tag, Rule())
                rules.p_rules[p_cls][tag] = _merge_rules(old, new_rule)
...
def render_visible_text(html_str: str, rules: Rules) -> tuple[str, list[str]]:
    ...
    def _apply_rule(el: html.HtmlElement, rule: Rule) -> str:
        ...
        # end char / attr
        if rule.append_end_char:
            s += rule.append_end_char
        if rule.append_end_attr:
            s += el.attrib.get(rule.append_end_attr, "")

        # start char / attr
        if rule.append_start_char:
            s = (rule.append_start_char or "") + s
        if rule.append_start_attr:
            s = el.attrib.get(rule.append_start_attr, "") + s

        if rule.transform_flip_x and s:
            refl_list.append(s)
        return s
    ...
    for p in tree.findall(".//p"):
        ...
        for child in p:
            ...
            if p_key is not None and child_tag in rules.orders:
                rule = rules.p_rules.get(p_key, {}).get(child_tag, Rule())
