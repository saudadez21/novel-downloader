...
class TaskManager:
            ...
            async with get_fetcher(task.site, fetcher_cfg) as fetcher:
                ...
                if getattr(downloader_cfg, "login_required", False):
                    ...
                    if not loaded or not getattr(fetcher, "is_logged_in", False):
                        login_ok = await self._handle_login(fetcher, downloader_cfg)
                        if not login_ok:
                            task.status = "failed"
                            task.error = "登录失败或已取消"
                            return
    ...
    async def _handle_login(
        self, fetcher: FetcherProtocol, downloader_cfg: DownloaderConfig
    ) -> bool:
        ...
        with ui.dialog() as dialog, ui.card().classes("w-[540px] max-w-full"):
            ...
            with ui.row().classes("justify-end w-full mt-2"):
                ...
                ui.button(
                    "登录", color="primary", on_click=lambda: _close_with_result(True)
                ).props("unelevated")
