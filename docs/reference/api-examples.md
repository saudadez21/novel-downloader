...
* **infra/**：基础设施 (配置、网络、路径/文件、日志、i18n、持久化)
* **libs/**：通用库 (EPUB、文件工具、加解密、时间工具)
...
```
plugins/sites/<site_key>/
```

**2. 必须实现的模块**:

* `fetcher.py`: 负责网络请求、获取书籍页与章节页
* `parser.py`: 负责解析为 `BookInfoDict` 与 `ChapterDict`

**3. 可选模块**:

* `client.py`: 若缺失则回退到 `common` 实现
...
* 在 `FetcherProtocol.login_fields` 中暴露字段 (`text` / `password` / `cookie`)
* `login()` 成功后需设置 `is_logged_in = True`
* 同时实现 `load_state()` / `save_state()` 以支持会话复用
