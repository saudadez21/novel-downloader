...
> 具体网站在渲染时是怎么去除重复内容的还有待研究

根据页面结构分析, 每个章节包含一个 `wordsCount` 字段, 用于标示正文展示字数。

通过样本验证, 网页端实际展示的内容长度 (排除空格换行等无效字符) 正好等于该字段值。

因此, 采用了**基于字数截断**的修复方式:

> **策略说明**：读取章节的 `wordsCount`, 在去除空格与换行后, 只保留正文中的前 `n` 个字符, 即可还原网页端展示的原始内容。
